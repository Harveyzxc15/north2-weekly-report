...
REGION_NAME  = '北二區'   # 合計列與結構檢查用；北一版為 '北一區'
...
def _fill_workbook(wk_end: date, log, use_full_month: bool = False,
    ...
    if LAUNCH:
        # 首三天窗固定 3 天，可能比累積窗還晚結束；兩個窗都要算進載入範圍，
        # 否則去年的第 3 天會被截掉
    ...
    def fill_acpp(ws, row_start, period):
        ...
        # Total 列台數/ACPP/SA 用 SUM（搭售率欄為公式，會自動依 SUM 重算）
    ...
    def fill_units(ws, row_start, pa, pb):
        for ri, code in enumerate(rows_stores):
            ...
            # ── 人流 (S=上期, V=本期) ──
            store_last = row_start + len(rows_stores) - 2   # 最後一個門市列
            if code in NO_TRAFFIC_COUNTER:
                # 無計數器門市：人流 = 成交筆數 × 0.85 / 0.3（去小數）
                ...
                # 有計數器門市：有人流快取就填，沒有就清空（手填，避免殘留範本內容）
                ta = TRAFFIC[pa].get(code, 0)
                tb = TRAFFIC[pb].get(code, 0)
                _set(ws, r, 19, ta if ta else None)
                _set(ws, r, 22, tb if tb else None)
            # 提袋率 = 成交筆數 / 人流
        ...
        # Total 列：台數/成交筆數用 SUM（人流已是SUM、平均單價為加權值保留）
    ...
    def fill_biz(ws, row_start, pa, pb):
        ...
        # Total 列加總欄用 SUM（差異/%/搭售率欄為公式，自動依 SUM 重算）
    ...
    def fill_biz_mo(ws, row_start, pa, pb):
        ...
        # Total 列加總欄用 SUM（達成率/差異/%欄為公式，自動依 SUM 重算）
        _total_sums(ws, row_start, [4,5,7,8,10,11,13,14,16,17,19,20,30,31])

    # ── 配件 sheets ──────────────────────────────────────────────
    log('填入配件 sheets…')
    fill_acc_section(wb['配件-北二區'], 4, 0, 'ALL')
    for code, sname in [('009','配件 - 永和門市'), ('025','配件 - 板橋誠品門市'),
                        ('050','配件 - 西門門市'), ('055','配件 - 花蓮門市'),
                        ('063','配件 - 板橋遠百門市'), ('064','配件 - 新莊宏匯門市'),
                        ('068','配件 - 新店裕隆城門市')]:
    ...
    fill_units(ws_wk, 15, 'pw', 'wk')
    fill_acpp(ws_wk, 25, 'wk');  fill_acpp(ws_wk, 35, 'pw')

    # ── BY店 月累積 ───────────────────────────────────────────────
    log('填入 BY店 月累積…')
    ws_mo = wb['BY店 月累積']
    fill_biz_mo(ws_mo, 4, 'pm_same', 'mtd')
    fill_units(ws_mo, 15, 'pm_same', 'mtd')
    fill_acpp(ws_mo, 25, 'mtd');  fill_acpp(ws_mo, 35, 'pm_same')

    # ── BY店 去年同期 ─────────────────────────────────────────────
    log('填入 BY店 去年同期…')
    ws_ly = wb['BY店 去年同期']
    fill_biz(ws_ly, 4, 'lymo', 'mtd')
    fill_units(ws_ly, 15, 'lymo', 'mtd')
    fill_acpp(ws_ly, 25, 'mtd');  fill_acpp(ws_ly, 35, 'lymo')

    # ── BY店 整年同期 ─────────────────────────────────────────────
    log('填入 BY店 整年同期…')
    ws_yoy = wb['BY店 整年同期']
    fill_biz(ws_yoy, 4, 'ytd_ly', 'ytd_cy')
    fill_units(ws_yoy, 15, 'ytd_ly', 'ytd_cy')
    fill_acpp(ws_yoy, 25, 'ytd_cy');  fill_acpp(ws_yoy, 35, 'ytd_ly')
    ...
    STAFF_BLOCKS = [('009',1),('025',23),('050',44),('055',67),
                    ('063',93),('064',116),('068',132)]
    ...
    for sname, label in [('配件-北二區','北二區'),
                          ('配件 - 永和門市','永和門市'), ('配件 - 板橋誠品門市','板橋誠品門市'),
                          ('配件 - 西門門市','西門門市'), ('配件 - 花蓮門市','花蓮門市'),
                          ('配件 - 板橋遠百門市','板橋遠百門市'), ('配件 - 新莊宏匯門市','新莊宏匯門市'),
                          ('配件 - 新店裕隆城門市','新店裕隆城門市')]:
    ...
    for _r, _c_pw, _c_wk in [(23, 2, 12), (34, 2, 18), (45, 2, 9), (56, 2, 18)]:
    ...
    _rep_w(ws_wk, 3, pw_lbl, wk_lbl);  _rep_w(ws_wk, 14, pw_lbl, wk_lbl)
    ws_wk.cell(24,1).value = wk_lbl;   ws_wk.cell(34,1).value = pw_lbl
    ...
    for c in range(1, ws_mo.max_column+1):
        if ws_mo.cell(14,c).value is None: continue
        # 26/27 = 平均單價的對照期/本期，早期漏列導致日期永遠停在範本殘值
        if c in {2,5,8,11,14,26}:  ws_mo.cell(14,c).value = pm_s_ymd
        elif c in {3,6,9,12,15,27}: ws_mo.cell(14,c).value = mtd_ymd
    ws_mo.cell(24,1).value = f'{MTD_START.year}\n{mtd_lbl}'
    ws_mo.cell(34,1).value = f'{PM_SAME_START.year}\n{pm_s_lbl}'
    ...
    for c in range(1, ws_ly.max_column+1):
        if ws_ly.cell(14,c).value is None: continue
        # 26/27 = 平均單價的對照期/本期，早期漏列導致日期永遠停在範本殘值
        if c in {2,5,8,11,14,26}:  ws_ly.cell(14,c).value = ly_ymd
        elif c in {3,6,9,12,15,27}: ws_ly.cell(14,c).value = cy_ymd
    ws_ly.cell(24,1).value = f'{MTD_START.year}\n{mtd_lbl}'
    ws_ly.cell(34,1).value = f'{LYMO_START.year}\n{ly_lbl}'
    ...
    for c in range(1, ws_yoy.max_column+1):
        if ws_yoy.cell(14,c).value is None: continue
        # 26/27 = 平均單價的對照期/本期，早期漏列導致日期永遠停在範本殘值
        if c in {2,5,8,11,14,26}:  ws_yoy.cell(14,c).value = ytd_ly_ymd
        elif c in {3,6,9,12,15,27}: ws_yoy.cell(14,c).value = ytd_cy_ymd
    ws_yoy.cell(24,1).value = f'{YTD_S_CY.year}\n{ytd_cy_lbl}'
    ws_yoy.cell(34,1).value = f'{YTD_S_LY.year}\n{ytd_ly_lbl}'
    # 第3列右半（3PP搭售率/SA Care搭售率/禮券金額）年份標籤：24/26/28=去年, 25/27/29=今年
...
def _verify_structure(wb, rows_stores, log):
    ...
    if 'BY店 本週其他細項' not in wb.sheetnames:
        ...
        ws_m = wb['BY店 本週其他細項']
        for first_row, label in [(3, 'SAcare檢測新機'), (14, '喇叭'), (25, 'iPhone配件'),
                                 (36, 'iPad配件'), (47, 'Watch配件'), (58, 'CPU配件')]:
...
def _run_job(job_id: str, wk_end_str: str, use_full_month: bool = False,
    ...
    try:
        ...
        filename = f'北二區週報_{wk_end}{suffix}.xlsx'
        with _LOCK:
...
def main():
    ...
    print(f'北二區週報產生器：http://127.0.0.1:{port}', flush=True)
