...
# ── 清掉佔用 8783 的舊伺服器（避免「跑到舊版」）──────────────
OLD=$(lsof -ti :8783 2>/dev/null)
if [ -n "$OLD" ]; then
  echo "發現舊伺服器仍在執行，先關閉…"
...
( sleep 2 && open -a "Google Chrome" "http://127.0.0.1:8783/" ) &

echo "啟動伺服器（port 8783）…"
echo "（瀏覽器會自動開啟，若沒開請手動前往 http://127.0.0.1:8783/ ）"
echo ""
echo "關閉此視窗即可停止伺服器。"
echo "-------------------------------------------"
python3 server.py
